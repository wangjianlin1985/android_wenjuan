...
public class UserInfoAddActivity extends Activity {
	...
	// 声明用户名输入框
	private EditText ET_userInfoname;
	// 声明登录密码输入框
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		ET_userInfoname = (EditText) findViewById(R.id.ET_userInfoname);
		...
		btnAdd.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				try {
					/*验证获取用户名*/
					if(ET_userInfoname.getText().toString().equals("")) {
						Toast.makeText(UserInfoAddActivity.this, "用户名输入不能为空!", Toast.LENGTH_LONG).show();
						ET_userInfoname.setFocusable(true);
						ET_userInfoname.requestFocus();
						return;
					}
					userInfo.setUserInfoname(ET_userInfoname.getText().toString());
					/*验证获取登录密码*/ 
					...
					if(ET_address.getText().toString().equals("")) {
						Toast.makeText(UserInfoAddActivity.this, "用户住址输入不能为空!", Toast.LENGTH_LONG).show();
